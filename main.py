...
#upload to mongoDB
client = pymongo.MongoClient("mongodb+srv://exampletest:exampletest123@cluster0.jxeye.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
with st.sidebar:
    ...
    """)
    st.markdown("---")
    st.info("💡 **Build your expertise in data warehousing and analysis with this tool!**")

# Main section
st.header("🔍 Enter YouTube Channel Details")
channel_id = st.text_input("Enter the YouTube Channel ID:")

if st.button("📥 Collect and Store Data"):
    if not channel_id:
        st.error("⚠️ Please enter a valid Channel ID!")
    else:
        ch_ids = []
        db = client["Youtube_data"]
        coll1 = db["channel_details"]
        ...
        if channel_id in ch_ids:
            st.success("✅ Channel details for the given ID already exist in the database.")
        else:
...
st.header("📜 Select a Channel for Analysis")
all_channels = []
coll1 = db["channel_details"]
for ch_data in coll1.find({}, {"_id": 0, "channel_information": 1}):
...
# Check if the 'videos' and 'channels' tables exist, create them if necessary
cursor.execute("SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'videos';")
videos_table_exists = cursor.fetchone()[0]

cursor.execute("SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'channels';")
channels_table_exists = cursor.fetchone()[0]
...
# If 'channels' table doesn't exist, create it
...
# Streamlit dropdown to select the question
...
# Handling the queries based on user selection
